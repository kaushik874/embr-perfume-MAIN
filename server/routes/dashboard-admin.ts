...
router.get("/dashboard", async (_req, res) => {
  ...
  const publishedProducts = ((await db.prepare("SELECT COUNT(*) as c FROM products WHERE status = 'published'").get()) as any).c;
  const draftProducts = ((await db.prepare("SELECT COUNT(*) as c FROM products WHERE status = 'draft'").get()) as any).c;

  const totalOrders = ((await db.prepare("SELECT COUNT(*) as c FROM orders").get()) as any).c;
  const todayOrders = ((await db.prepare("SELECT COUNT(*) as c FROM orders WHERE date(created_at) = date('now')").get()) as any).c;
  const pendingOrders = ((await db.prepare("SELECT COUNT(*) as c FROM orders WHERE status = 'pending'").get()) as any).c;
  const paidOrders = ((await db.prepare("SELECT COUNT(*) as c FROM orders WHERE status = 'paid'").get()) as any).c;
  const shippedOrders = ((await db.prepare("SELECT COUNT(*) as c FROM orders WHERE status = 'shipped'").get()) as any).c;
  const deliveredOrders = ((await db.prepare("SELECT COUNT(*) as c FROM orders WHERE status = 'delivered'").get()) as any).c;
  const cancelledOrders = ((await db.prepare("SELECT COUNT(*) as c FROM orders WHERE status = 'cancelled'").get()) as any).c;
